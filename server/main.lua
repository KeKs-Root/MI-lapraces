...
ESX.RegisterServerCallback('MI-lapraces:server:GetRacingLeaderboards', function(source, cb)
    cb(Races)
end)
...
RegisterServerEvent('MI-lapraces:server:FinishPlayer')
AddEventHandler('MI-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    local src = source
    ...
    TriggerClientEvent('MI-lapraces:client:PlayerFinishs', -1, RaceData.RaceId, PlayersFinished, esxChar)
...
RegisterServerEvent('MI-lapraces:server:CreateLapRace')
AddEventHandler('MI-lapraces:server:CreateLapRace', function(RaceName)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    
    if IsNameAvailable(RaceName) then
         TriggerClientEvent('MI-lapraces:client:StartRaceEditor', source, RaceName)
    else
        TriggerClientEvent('notification', source, 'There is already a race with this name.', 2)
    end
end)

ESX.RegisterServerCallback('MI-lapraces:server:GetRaces', function(source, cb)
    cb(AvailableRaces)
end)

ESX.RegisterServerCallback('MI-lapraces:server:GetListedRaces', function(source, cb)
    cb(Races)
end)

ESX.RegisterServerCallback('MI-lapraces:server:GetRacingData', function(source, cb, RaceId)
    cb(Races[RaceId])
end)

ESX.RegisterServerCallback('MI-lapraces:server:HasCreatedRace', function(source, cb)
    cb(HasOpenedRace(ESX.GetPlayerFromId(source).identifier))
end)

ESX.RegisterServerCallback('MI-lapraces:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
    cb(true)
end)
...
ESX.RegisterServerCallback('MI-lapraces:server:GetTrackData', function(source, cb, RaceId)
...
RegisterServerEvent('MI-lapraces:server:JoinRace')
AddEventHandler('MI-lapraces:server:JoinRace', function(RaceData)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    local esxChar = GetCharacterName(src)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('MI-lapraces:client:LeaveRace', src, Races[CurrentRace])
        else
    ...
    TriggerClientEvent('MI-lapraces:client:JoinRace', src, Races[RaceId], RaceData.Laps)
    TriggerClientEvent('qb-phone:client:UpdateLapraces', -1)
...
RegisterServerEvent('MI-lapraces:server:LeaveRace')
AddEventHandler('MI-lapraces:server:LeaveRace', function(RaceData)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    local esxChar = GetCharacterName(src)
    local RaceName
    ...
    if (AmountOfRacers - 1) == 0 then
        ...
        table.remove(AvailableRaces, AvailableKey)
        TriggerClientEvent('notification', src, 'You were the only one in the race. The race is over.', 2)
        TriggerClientEvent('MI-lapraces:client:LeaveRace', src, Races[RaceId])
...
RegisterServerEvent('MI-lapraces:server:SetupRace')
AddEventHandler('MI-lapraces:server:SetupRace', function(RaceId, Laps)
                    ...
                    if Races[RaceId].Waiting then
                        ...
                        for cid,_ in pairs(Races[RaceId].Racers) do
                            ...
                            if RacerData ~= nil then
                                TriggerClientEvent('MI-lapraces:client:LeaveRace', RacerData.source, Races[RaceId])
                            end
                        end
                        table.remove(AvailableRaces, AvailableKey)
...
RegisterServerEvent('MI-lapraces:server:UpdateRaceState')
AddEventHandler('MI-lapraces:server:UpdateRaceState', function(RaceId, Started, Waiting)
    Races[RaceId].Waiting = Waiting
    Races[RaceId].Started = Started
end)

RegisterServerEvent('MI-lapraces:server:UpdateRacerData')
AddEventHandler('MI-lapraces:server:UpdateRacerData', function(RaceId, Checkpoint, Lap, Finished)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    ...
    TriggerClientEvent('MI-lapraces:client:UpdateRaceRacerData', -1, RaceId, Races[RaceId])
end)

RegisterServerEvent('MI-lapraces:server:StartRace')
AddEventHandler('MI-lapraces:server:StartRace', function(RaceId)
    local src = source
    ...
    if RaceId ~= nil then
        if AvailableRaces[AvailableKey].SetupSteam == MyPlayer.identifier then
            ...
            for identifier,_ in pairs(Races[RaceId].Racers) do
                ...
                if Player ~= nil then
                    TriggerClientEvent('MI-lapraces:client:RaceCountdown', Player.source)
                end
            end
            TriggerClientEvent('qb-phone:client:UpdateLapraces', -1)
        else
...
RegisterServerEvent('MI-lapraces:server:SaveRace')
AddEventHandler('MI-lapraces:server:SaveRace', function(RaceData)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
...
ESX.RegisterServerCallback('MI-lapraces:server:CanRaceSetup', function(source, cb)
