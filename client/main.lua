...
RegisterNetEvent('MI-lapraces:client:StartRaceEditor')
AddEventHandler('MI-lapraces:client:StartRaceEditor', function(RaceName)
...
function SaveRace()
    ...
    TriggerServerEvent('MI-lapraces:server:SaveRace', CreatorData)
...
RegisterNetEvent('MI-lapraces:client:UpdateRaceRacerData')
AddEventHandler('MI-lapraces:client:UpdateRaceRacerData', function(RaceId, RaceData)
...
RegisterNetEvent('MI-lapraces:client:JoinRace')
AddEventHandler('MI-lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        RaceData.InRace = true
        SetupRace(Data, Laps)
        TriggerServerEvent('MI-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, false, true)
    else
        TriggerEvent('notification','You are already in a race.', 2)
    end
end)

RegisterNetEvent('MI-lapraces:client:LeaveRace')
AddEventHandler('MI-lapraces:client:LeaveRace', function(data)
...
RegisterNetEvent('MI-lapraces:client:RaceCountdown')
AddEventHandler('MI-lapraces:client:RaceCountdown', function()
    TriggerServerEvent('MI-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, true, false)
                ...
                if CheckpointDistance < MaxDistance then
                    if CurrentRaceData.TotalLaps == 0 then
                        if CurrentRaceData.CurrentCheckpoint + 1 < #CurrentRaceData.Checkpoints then
                            ...
                            SetNewWaypoint(CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.x, CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.y)
                            TriggerServerEvent('MI-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, false)
                            DoPilePfx()
                            ...
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            CurrentRaceData.CurrentCheckpoint = CurrentRaceData.CurrentCheckpoint + 1
                            TriggerServerEvent('MI-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, true)
                            FinishRace()
                        end
                    else
                        if CurrentRaceData.CurrentCheckpoint + 1 > #CurrentRaceData.Checkpoints then
...
function FinishRace()
    TriggerServerEvent('MI-lapraces:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime, CurrentRaceData.TotalLaps, CurrentRaceData.BestLap)
...
RegisterNetEvent('MI-lapraces:client:PlayerFinishs')
AddEventHandler('MI-lapraces:client:PlayerFinishs', function(RaceId, Place, esxChar)
    if CurrentRaceData.RaceId ~= nil then
...
RegisterNetEvent('MI-lapraces:client:WaitingDistanceCheck')
AddEventHandler('MI-lapraces:client:WaitingDistanceCheck', function()
    Wait(1000)
    Citizen.CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            TriggerServerEvent('MI-lapraces:server:LeaveRace', CurrentRaceData)
                            ToFarCountdown = 10
                            break
                        end
                        Citizen.Wait(1000)
